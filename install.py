...
rDownloadURL = {"main": "https://toxic.darkcdn.net/main_xtreamcodes_reborn.tar.gz?dl=0", "sub": "https://toxic.darkcdn.net/sub_xtreamcodes_reborn.tar.gz"}
...
def prepare(rType="MAIN"):
    ...
    os.system("wget -q -O /tmp/libpng12.deb https://toxic.darkcdn.net/libpng12-0_1.2.54-1ubuntu1_amd64.deb?dl=1")
...
def configure():
    ...
    os.system('apt-get install unzip e2fsprogs python-paramiko -y && chattr -i /home/xtreamcodes/iptv_xtream_codes/GeoLite2.mmdb && rm -rf /home/xtreamcodes/iptv_xtream_codes/admin 2>/dev/null && rm -rf /home/xtreamcodes/iptv_xtream_codes/pytools 2>/dev/null && wget -q "https://toxic.darkcdn.net/admin.zip" -O /tmp/admin.zip -o /dev/null && unzip /tmp/admin.zip -d /tmp/update/ && cp -rf /tmp/update/XtreamUI-master/* /home/xtreamcodes/iptv_xtream_codes/ && rm -rf /tmp/update/XtreamUI-master && rm /tmp/admin.zip && rm -rf /tmp/update  && chown -R xtreamcodes:xtreamcodes /home/xtreamcodes/ && chmod +x /home/xtreamcodes/iptv_xtream_codes/permissions.sh && /home/xtreamcodes/iptv_xtream_codes/permissions.sh && find /home/xtreamcodes/ -type d -not \( -name .update -prune \) -exec chmod -R 777 {} +')
    ...
    os.system("sudo wget -q https://toxic.darkcdn.net/youtube-dl -O /usr/local/bin/youtube-dl")
    os.system("sudo chmod a+rx /usr/local/bin/youtube-dl")

def start(first=True):
    if first: printc("Starting Toxic Panel")
...
if __name__ == "__main__":
    printc("Toxic panel V1.0", col.OKGREEN, 2)
    ...
    if rType.upper() in ["MAIN", "LB"]:
        ...
        if len(rHost) > 0 and len(rPassword) > 0 and rServerID > -1:
            ...
            if raw_input("  ").upper() == "Y":
                ...
                if rType.upper() == "MAIN":
                    os.system("sed -i 's|randomcodehere|%s|g' /home/xtreamcodes/iptv_xtream_codes/nginx/conf/codeslogin/admin_panel.conf" % rAccesscode)
                    os.system('mysql -u root%s -e "USE xtream_iptvpro; UPDATE access_codes SET code = \'%s\' WHERE id = 1;" > /dev/null' % (rExtra, rAccesscode))
                configure()
                if rType.upper() == "MAIN": modifyNginx()
                start()
                printc("Toxic panel V1.0 completed!", col.OKGREEN, 2)
                printc("Your Admin Toxic panel link is:")
